...
export default class pathFinder {
    ...
    drawPath(ct) {
        ...
        for (let i = 1; i < this.paths.length; i++) {
            ...
            let row = Math.min(this.paths[i - 1].row, this.paths[i].row)
            let column = Math.min(this.paths[i - 1].column, this.paths[i].column)
            
            if (this.paths[i - 1].column == this.paths[i].column) {
                ct.fillRect(this.paths[i].column * this.mul + 10, row * this.mul + 10, this.size, this.mul)
            }
            if (this.paths[i - 1].row == this.paths[i].row) {
    ...
    visualizePath(ct) {
        ...
        function animate(redo = false, n = cur.iter+1) {
            if (n == cur.paths.length) {
                ...
                let temp;
                let row = Math.min(paths[n - 1].row, paths[n].row)
                let column = Math.min(paths[n - 1].column, paths[n].column)
                
                if (paths[n - 1].column ==paths[n].column) {
                    ct.fillRect(paths[n].column * mul + 10, row * mul + 10, size, mul)
                }
                if (paths[n - 1].row == paths[n].row) {
